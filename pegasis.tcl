...
source /home/awaliap/ns-allinone-2.34/ns-2.34/mit/uAMPS/ns-pegasis.tcl

set opt(rcapp)        "Application/PEGASIS"     ;# Application type
set opt(tr)           "trace/pegasis.tr"   ;# Trace file
# Need to spread the data by k+1
set opt(spreading)    [expr $opt(num_clusters)+1]

set outf [open "/home/awaliap/ns-allinone-2.34/ns-2.34/mit/leach_sims/test_simulations/pegasis/conditions.txt"]
puts $outf "USING PEGASIS"
close $outf

source uAMPS/sims/uamps.tcl
...
set outf [open "$opt(dirname)/conditions.txt"]
